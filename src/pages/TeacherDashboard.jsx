...
export default function TeacherDashboard() {
  ...
  const handleCreateRoom = async (e) => {
    ...
    // Generate random 8-character uppercase room code
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header Banner */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-indigo-900/40 via-slate-900 to-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 backdrop-blur-xl relative overflow-hidden mb-8 shadow-2xl">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-sm text-slate-400">Loading your classrooms...</p>
          </div>
        ) : rooms.length === 0 ? (
          <div className="bg-slate-900/50 border border-dashed border-slate-800 rounded-3xl p-12 text-center flex flex-col items-center justify-center">
            ...
            <h3 className="text-lg font-bold text-white mb-1">No classrooms created yet</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {rooms.map((room) => (
              <div
                key={room.id}
                className="bg-slate-900/80 border border-slate-800 hover:border-slate-700 rounded-3xl p-6 transition-all hover:shadow-2xl hover:shadow-indigo-500/5 flex flex-col justify-between group"
              >
                <div>
                  ...
                  {/* Room Code Badge */}
                  <div className="mt-4 flex items-center justify-between p-3 rounded-2xl bg-slate-800/80 border border-slate-700/60">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-xs px-3 py-1.5 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 font-medium transition-colors"
                    >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative">
            <h3 className="text-xl font-bold text-white mb-2">Create New Class</h3>
            ...
            <form onSubmit={handleCreateRoom} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="block w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-all"
                />
              </div>
